...
sequenceDiagram
    ...
    Note over OperatorA, OperatorB: Trigger Checkpoint (e.g., by JobManager)
    ...
    Note over OperatorA, OperatorB: Failure Occurs!

    Note over OperatorA, OperatorB: Restart from Checkpoint
...
| 优化维度 | 关键挑战             | 常见优化策略                                                                                                 |
| :------- | :------------------- | :----------------------------------------------------------------------------------------------------------- |
| I/O      | 磁盘/网络瓶颈        | 内存计算, 列式存储, 谓词下推, 分区裁剪, 高效序列化, 压缩, 异步I/O, 并行I/O                                  |
| Network  | Shuffle 开销大       | Map端预聚合, Broadcast Join, 优化Shuffle机制, 高效序列化, 压缩, Netty优化                                     |
| CPU      | 计算密集, GC 开销    | 代码生成, 向量化执行, 算子优化, 减少GC (堆外内存)                                                              |
...
graph LR
    ...
    subgraph Flink Abstractions
        ...
        TableAPI_SQL -- Optimized by --> Flink Optimizer[Flink Optimizer]
    end
